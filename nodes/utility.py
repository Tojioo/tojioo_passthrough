...
def create_utility_nodes(category: str, node_ui_specs: Dict[str, Dict[str, Any]]) -> Dict[str, type]:
	"""
	...
	def _dynamic_optional(node_key: str):
		...
		# For PT_DynamicPassthrough, base_name is "input" (no trailing digit).
		# Derive prefix by stripping trailing digits from base_name.
		prefix = "".join(ch for ch in base_name if not ch.isdigit())
		# If base_name has no digits (e.g. "input"), prefix == base_name.
		# We need to accept both "input" and "input_2", "input_3", etc.

		class DynamicOptional(dict):
			def __contains__(self, key):
				if isinstance(key, str):
					# Accept exact base_name OR keys starting with prefix followed by "_" or digit
					if key == base_name:
						return True
					if key.startswith(prefix + "_") or (key.startswith(prefix) and len(key) > len(prefix) and key[len(prefix)].isdigit()):
						return True
	...
	utility_nodes: Dict[str, type] = {}

	if "PT_DynamicPassthrough" in node_ui_specs:
		class PT_DynamicPassthrough:
			"""
			Dynamic multi-IO passthrough.
	
			- Inputs are dynamic: input, input_2, input_3, ...
			- Outputs mirror inputs: output, output_2, output_3, ...
			- Each output slot returns the corresponding input unchanged.
			"""
			DESCRIPTION = "Dynamic passthrough with one output per input; types adapt based on connections."
			_NODE_SPEC = node_ui_specs["PT_DynamicPassthrough"]
			NODE_NAME = _NODE_SPEC.get("node_name", "Dynamic Passthrough")
	
			_MAX_SOCKETS = 32
	
			@classmethod
			def INPUT_TYPES(cls):
				opt = _dynamic_optional("PT_DynamicPassthrough")
				return {"required": {}, "optional": opt}
	
			RETURN_TYPES = tuple("*" for _ in range(_MAX_SOCKETS))
			RETURN_NAMES = ("output",) + tuple(f"output_{i}" for i in range(2, _MAX_SOCKETS + 1))
			FUNCTION = "run"
			CATEGORY = category
	
			def run(self, **kwargs):
				out = [kwargs.get("input")]
				for i in range(2, self._MAX_SOCKETS + 1):
					out.append(kwargs.get(f"input_{i}"))
				return tuple(out)
	
		utility_nodes["PT_DynamicPassthrough"] = PT_DynamicPassthrough

	if "PT_AnyImageBatchSwitch" in node_ui_specs:
		...
		utility_nodes["PT_AnyImageBatchSwitch"] = PT_AnyImageBatchSwitch

	if "PT_AnyMaskBatchSwitch" in node_ui_specs:
		...
		utility_nodes["PT_AnyMaskBatchSwitch"] = PT_AnyMaskBatchSwitch

	if "PT_AnyLatentBatchSwitch" in node_ui_specs:
		...
		utility_nodes["PT_AnyLatentBatchSwitch"] = PT_AnyLatentBatchSwitch

	if "PT_AnyConditioningBatchSwitch" in node_ui_specs:
		...
		utility_nodes["PT_AnyConditioningBatchSwitch"] = PT_AnyConditioningBatchSwitch
